...
import com.smartfarmers.adapters.MessageAdapter;
...
public class ChatActivity extends BaseActivity {
    ...
    private String taggedProductId, taggedProductName, taggedProductImage;
    private double taggedProductPrice;
    private androidx.cardview.widget.CardView cardTaggedPost;
    private android.widget.ImageView ivTaggedPostImage;
    private android.widget.TextView tvTaggedPostName, tvTaggedPostPrice;
    private android.widget.ImageButton btnCloseTaggedPost;
    private SupabaseAuthHelper supabaseAuth;
    private SessionManager sessionManager;
    private com.smartfarmers.models.ChatDatabase localDb;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        taggedProductId = getIntent().getStringExtra("tagged_product_id");
        taggedProductName = getIntent().getStringExtra("tagged_product_name");
        taggedProductImage = getIntent().getStringExtra("tagged_product_image");
        taggedProductPrice = getIntent().getDoubleExtra("tagged_product_price", 0.0);

        supabaseAuth = new SupabaseAuthHelper(this);
        sessionManager = new SessionManager(this);
        localDb = com.smartfarmers.models.ChatDatabase.getInstance(this);
    ...
    private void initViews() {
        ...
        cardTaggedPost = findViewById(R.id.cardTaggedPost);
        ivTaggedPostImage = findViewById(R.id.ivTaggedPostImage);
        tvTaggedPostName = findViewById(R.id.tvTaggedPostName);
        tvTaggedPostPrice = findViewById(R.id.tvTaggedPostPrice);
        btnCloseTaggedPost = findViewById(R.id.btnCloseTaggedPost);

        btnSend.setOnClickListener(v -> sendMessage());
        btnAttach.setOnClickListener(v -> selectMedia());
        btnCloseTaggedPost.setOnClickListener(v -> cardTaggedPost.setVisibility(View.GONE));
    }

    private void setupTaggedPost() {
        if (taggedProductId != null && !taggedProductId.isEmpty()) {
            cardTaggedPost.setVisibility(View.VISIBLE);
            tvTaggedPostName.setText(taggedProductName);
            tvTaggedPostPrice.setText(String.format(java.util.Locale.US, "Rs. %.2f", taggedProductPrice));
            
            if (taggedProductImage != null && !taggedProductImage.isEmpty()) {
                try {
                    byte[] imageBytes = android.util.Base64.decode(taggedProductImage, android.util.Base64.DEFAULT);
                    com.bumptech.glide.Glide.with(this)
                            .load(imageBytes)
                            .into(ivTaggedPostImage);
                } catch (Exception e) {
                    ivTaggedPostImage.setImageResource(android.R.drawable.ic_menu_gallery);
                }
            } else {
                ivTaggedPostImage.setImageResource(android.R.drawable.ic_menu_gallery);
            }

            ivTaggedPostImage.setOnClickListener(v -> {
                Intent intent = new Intent(this, FullscreenImageActivity.class);
                intent.putExtra("image_data", taggedProductImage);
                startActivity(intent);
            });

            // Optional: Auto-fill the message with a reference to the post
            etMessage.setText(getString(R.string.interested_in_product, taggedProductName));
        }
    }
    ...
    private void sendMediaMessage(String type, String base64) {
        try {
            ...
            supabaseAuth.sendDirectMessage(json, new com.smartfarmers.auth.SupabaseAuthHelper.AuthCallback() {
                @Override
    ...
    private void updateToolbarInfo() {
        if (otherUserId != null) {
            ...
            supabaseAuth.getUserProfile(otherUserId, new com.smartfarmers.auth.SupabaseAuthHelper.AuthCallback() {
                @Override
            ...
            supabaseAuth.fetchChatRoomDetails(roomId, new com.smartfarmers.auth.SupabaseAuthHelper.AuthCallback() {
                @Override
                public void onSuccess(String json) {
                    try {
    ...
    private void fetchMessages() {
        ...
        new Thread(() -> {
            try {
                List<com.smartfarmers.models.MessageEntity> localMsgs;
                ...
                if (!localMsgs.isEmpty()) {
                    runOnUiThread(() -> {
                        ...
                        for (com.smartfarmers.models.MessageEntity entity : localMsgs) {
        ...
        com.smartfarmers.auth.SupabaseAuthHelper.AuthCallback callback = new com.smartfarmers.auth.SupabaseAuthHelper.AuthCallback() {
            @Override
            public void onSuccess(String json) {
                try {
                    JSONArray arr = new JSONArray(json);
                    List<Message> list = new ArrayList<>();
                    List<com.smartfarmers.models.MessageEntity> entities = new ArrayList<>();
                    ...
                    for (int i = 0; i < arr.length(); i++) {
                        ...
                        entities.add(new com.smartfarmers.models.MessageEntity(
                    ...
                    new Thread(() -> {
                        ...
                        for (com.smartfarmers.models.MessageEntity entity : entities) {
    ...
    private String getChatSeed() {
        if (otherUserId != null && !otherUserId.isEmpty()) {
            return com.smartfarmers.utils.EncryptionUtils.getConversationSeed(sessionManager.getUserId(), otherUserId);
        }
        return roomId;
    }
    ...
    private void sendMessage() {
        ...
        if (roomId != null && roomId.startsWith("GRP-")) {
            supabaseAuth.addParticipant(roomId, sessionManager.getUserId(), "joined", "member", new com.smartfarmers.auth.SupabaseAuthHelper.AuthCallback() {
    ...
    private void performSendMessage(String text) {
        ...
        String encryptedText = com.smartfarmers.utils.EncryptionUtils.encrypt(text, seed);
