...
import com.smartfarmers.adapters.ChatRoomAdapter;
import com.smartfarmers.auth.SupabaseAuthHelper;
import com.smartfarmers.models.ChatRoom;
...
public class CommunityActivity extends BaseActivity {
    ...
    private void fetchPersonalChats() {
        ...
        new Thread(() -> {
            List<com.smartfarmers.models.MessageEntity> allLocal = localDb.messageDao().getAllMessages();
            java.util.Map<String, ChatRoom> localUnique = new java.util.LinkedHashMap<>();
            String myId = sessionManager.getUserId();
            
            for (com.smartfarmers.models.MessageEntity msg : allLocal) {
                ...
                if (!localUnique.containsKey(otherId)) {
                    ...
                    room.setGroup(false);
                    String seed = com.smartfarmers.utils.EncryptionUtils.getConversationSeed(myId, otherId);
                    room.setLastMessage(com.smartfarmers.utils.EncryptionUtils.decrypt(msg.getMessageText(), seed));
                    ...
                    try {
                        ...
                        for (int i = 0; i < arr.length(); i++) {
                            ...
                            if (!otherId.isEmpty() && !otherId.equals(myId) && !uniquePersonal.containsKey(otherId)) {
                                ...
                                room.setGroup(false);
                                String seed = com.smartfarmers.utils.EncryptionUtils.getConversationSeed(myId, otherId);
                                room.setLastMessage(com.smartfarmers.utils.EncryptionUtils.decrypt(obj.optString("message_text", ""), seed));
