...
import com.smartfarmers.adapters.BarterRequestAdapter;
...
public class BarterRequestFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        adapter = new BarterRequestAdapter(requests, isReceivedType, new BarterRequestAdapter.OnRequestActionListener() {
            ...
            @Override
            public void onChat(BarterRequest request) {
                String otherUserId = isReceivedType ? request.getRequesterId() : request.getOwnerId();
                if (otherUserId == null || otherUserId.isEmpty()) {
                    Toast.makeText(getContext(), "User information missing", Toast.LENGTH_SHORT).show();
                    return;
                }

                android.content.Intent intent = new android.content.Intent(getContext(), com.smartfarmers.activities.ChatActivity.class);
                intent.putExtra("other_user_id", otherUserId);
                intent.putExtra("room_name", request.getRequesterName());
                intent.putExtra("tagged_product_id", request.getListingId());
                intent.putExtra("tagged_product_name", "Barter: " + request.getListingTitle());
                intent.putExtra("tagged_product_image", ""); // You could add image if available in model
                    ...
                    try {
                        ...
                        for (int i = 0; i < arr.length(); i++) {
                            ...
                            if (obj.has("barter_listings") && !obj.isNull("barter_listings")) {
                                ...
                                req.setOwnerId(listing.optString("user_id", ""));
                                ...
                                req.setOwnerId(obj.optString("owner_id", ""));
                            }
